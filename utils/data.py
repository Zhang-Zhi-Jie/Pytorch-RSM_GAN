...
def load_data(opt):
    ...
    test_file_list = os.listdir(train_data_path)
    ...
    dataset["train"] = torch.from_numpy(np.array(train_data))
    dataset["test"] = torch.from_numpy(np.array(test_data))

    dataloader = {x: torch.utils.data.DataLoader(
                                dataset=dataset[x], batch_size=opt.batch_size, shuffle=shuffle[x]) 
                                for x in splits}
    return dataloader